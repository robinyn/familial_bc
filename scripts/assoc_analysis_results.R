...
all_three=Reduce(intersect, list(t1$SNP,t2$SNP,t3$SNP))

samples = european_subset_significant %>% 
  filter(OR>1) %>% 
  dplyr::select(samples) %>% 
  separate_longer_delim(samples, "|") %>% 
  unique()

samples = test %>% filter(variant=="11-108150275-G-A") %>% 
  dplyr::select(samples) %>% 
  separate_longer_delim(samples, "|") %>% 
  unique()

samples = case_phenotypes %>% filter(BRIDGES_ID %in% samples$samples)
