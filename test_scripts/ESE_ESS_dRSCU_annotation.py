...
# Title: ESE/ESS/dRSCU Annotation script for VCF files
#
# Description: This script is a modified version of the custom_annotation.py script by Arthur Boffelli Castro.
#   The script annotates exonic splicing enhancer/silencer motifs overlapping a given variant, as well as the
...
#
# Original script by Arthur can be found in https://www.github.com/aboffelli/variant_annotation
...
def reformat_lines(line):
    ...
    for transcript_csq in csq:
        ...
        dRSCU = calculate_dRSCU(transcript_csq)
        ESE_EES = annotate_ESE_ESS(transcript_csq, flanking_sequence)
...
def calculate_dRSCU(transcript_csq):
...
def annotate_ESE_ESS(transcript_csq, flanking_sequence):
...
ese_set, ess_set, rscu_table, list_of_files = initialize()
